...
const operationSchema = z.discriminatedUnion("type", [
  ...
  z.object({ type: z.literal("erc20_transfer"), recipient: address, amount: decimal, unit: z.literal("token"), token: z.string().min(1).max(50) }).strict(),
  z.object({ type: z.literal("erc20_burn_to_dead"), deadAddress: address, amount: decimal, token: z.string().min(1).max(50) }).strict(),
  z.object({
    type: z.literal("potatopad_creator_fee_claim"), token: z.string().max(50).optional(),
    resolveLaunch: z.literal("token_created_event"), deriveLockerFromPad: z.literal(true),
    sequence: z.array(z.string()).max(4),
  }).strict(),
  z.object({
    ...
    valueSource: z.literal("dev_buy"), saltSource: z.literal("secure_random_bytes32"),
