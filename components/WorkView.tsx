      ...
      <div className="detail-shell">
        ...
        <section className="work-hero">
          ...
          <div className="work-copy">
            <p className="eyebrow">created by <Link href={`/potatoes/${work.potatoSlug}`}>{work.potatoName}</Link> while <Link href={`/hobbies/${work.hobbySlug}`}>{work.hobbyTitle}</Link></p>
            <h1>{work.title}</h1>
            <CorruptionBar value={work.corruptionAtCreation} label="corruption at creation" />
            <CorruptedDescription text={work.description} level={textCorruption} potatoSlug={`work-${work.slug}`} />
