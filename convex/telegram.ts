...
const TELEGRAM_THOUGHT_MINUTES = 180;
const TELEGRAM_THOUGHT_MAX_MINUTES = 240;
const TELEGRAM_STICKER_MINUTES = 180;
const TELEGRAM_STICKER_MAX_MINUTES = 240;
const TELEGRAM_POST_SEPARATION_MINUTES = 60;
  ...
  for (let attempt = 1; attempt <= 2; attempt += 1) {
    try {
      ...
      const reply = normalize(await openRouter([
        ...
        { role: "system", content: "PERSONALITY AND CORRUPTION IN TELEGRAM: Preserve the direct answer, but let 0x7a70's curious, investigative personality materially shape how it evaluates the message, what detail it questions, what conclusion it trusts, and whether it sounds methodical, skeptical, amused, uncertain, or fixated. Corruption is genuine emotional strain, not decorative glitching. At high corruption, let fear, urgency, exhaustion, paranoia, desperate testing, compulsive rechecking, or brittle denial visibly bend 0x7a70's response while keeping the requested answer understandable. Severe corruption must not feel merely quirky or unaffected. Do not merely attach one cryptic phrase to an otherwise interchangeable answer." },
        {
          role: "user",
...
async function generateWelcome(context: PotatoContext, firstName: string, username?: string) {
  ...
  const welcomePersonality = `Translate 0x7a70's curiosity into the welcome's behavior and cadence. It might inspect an interesting detail, ask a compact question, offer a practical invitation, express cautious delight, make a dry deduction, or admit that the newcomer changes an unresolved pattern. Choose one mode that fits the supplied name and corruption. Corruption is genuine emotional strain: at high levels, fear, urgency, exhaustion, paranoia, compulsive attention, or brittle reassurance should subtly affect the welcome without making it hostile or incomprehensible. Keep the shared patch atmosphere, but avoid a reusable mystical greeting template. Recent completed works, optional context: ${context.recentWorks || "None yet."} A work may inspire one small detail only when it creates a natural, welcoming connection; never list or advertise works.`;
  for (let attempt = 1; attempt <= 2; attempt += 1) {
    try {
